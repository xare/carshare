...
#
# ***** END LICENSE BLOCK *****/
// WE create a locationSuggest object from the google.maps.Geocoder library class
var locationSuggest = new google.maps.Geocoder();

//We define the method _update it takes the input object from which it's value is extracted'
locationSuggest._update = function (input) {
  ...
  //The .goecode method takes the query as an address and focusing on greece
  locationSuggest._waitingDelay = setTimeout(function(){
    locationSuggest.geocode({address: query, region: "gr"}, locationSuggestesult);
  }, 300);
...
locationSuggest._updateInput = function (input, googleGeocodeItem, suggest) {
  if (suggest) {
    // TODO: do fancy suggest autocompletion!
      ...
      //The googleGeocodeItem has been returned as a response from google via json format, and one line in the
      // json response is formatted_address
    input.value = googleGeocodeItem.formatted_address;
    if(input.id=='from')
        $('input[name="fromLatLon"]').val(googleGeocodeItem.geometry.location);
    if(input.id=='to')
        $('input[name="toLatLon"]').val(googleGeocodeItem.geometry.location);
    //alert(dodo);
    //$("#fromLanLon").val(googleGeocodeItem.geometry.location);
  }
